...
NSString *const GCDAsyncSocketSSLClientSideAuthentication = @"GCDAsyncSocketSSLClientSideAuthentication";
#if !TARGET_OS_IPHONE
...
- (void)ssl_startTLS
{
	...
	if (shouldManuallyEvaluateTrust)
	{
		status = SSLSetSessionOption(sslContext,
                                     isServer ? kSSLSessionOptionBreakOnClientAuth
    ...
    NSNumber* sslAuth = tlsSettings[GCDAsyncSocketSSLClientSideAuthentication];
    if (sslAuth && isServer)
    {
        status = SSLSetClientSideAuthenticate(sslContext, (SSLAuthenticate)sslAuth.intValue);
        if (status != noErr)
        {
...
- (void)ssl_shouldTrustPeer:(BOOL)shouldTrust stateIndex:(int)aStateIndex
{
	...
	if (shouldTrust)
	{
        ...
        // Continue handshake if it hasn't completed already.
        if ((flags & kStartingReadTLS) || (flags & kStartingWriteTLS))
        {
            [self ssl_continueSSLHandshake];
        }
